...
INT64 __fastcall NtUserQueryDisplayConfig_hk(ULONG64 a1) {
	...
	auto info = (PINFORMATION)ExAllocatePool(NonPagedPool, sizeof(INFORMATION));
	...
	if (info) {
		...
		case 0:
			PsLookupProcessByProcessId((HANDLE)info->process_id, &target_process);
			LOG("[>]Attach to process: 0x%llx\n", info->process_id);
			break;
		...
		case 1:
			status =
				MmCopyVirtualMemory(target_process,
					info->target,
					PsGetCurrentProcess(),
					info->buffer, info->size,
					KernelMode,
					&bytes_copied);

			info->return_size = bytes_copied;
...
EXTERN_C NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObject, PUNICODE_STRING RegistryPath)
{
	...
	auto data_ptr = ((ULONG64)win32k_base + 0x677A8);
